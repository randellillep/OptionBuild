...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const optimizeStrategySchema = z.object({
    symbol: z.string(),
    ...
    metrics: z.object({
      ...
      netPremium: z.number(),
      isMaxProfitUnlimited: z.boolean().optional(),
      isMaxLossUnlimited: z.boolean().optional(),
    }).nullable().optional(),
  });

  app.post("/api/optimize-strategy", async (req, res) => {
    try {
      const validation = optimizeStrategySchema.safeParse(req.body);
      ...
      const { symbol, currentPrice, volatility, legs, greeks, metrics } = validation.data;

      if (!OPENAI_API_KEY) {
        return res.status(503).json({ 
          error: "OpenAI API key not configured. Please add OPENAI_API_KEY to use the AI Strategy Optimizer." 
        });
      }

      // Build the strategy description for AI analysis
      const legDescriptions = legs.map(leg => 
        `${leg.position.toUpperCase()} ${leg.quantity}x ${leg.type.toUpperCase()} @ $${leg.strike} strike, premium $${leg.premium.toFixed(2)}`
      ).join("\n");
...
- Theta: ${greeks.theta.toFixed(2)} (daily decay)
- Vega: ${greeks.vega.toFixed(2)}

${metrics ? `Risk Metrics:
- Max Profit: ${metrics.isMaxProfitUnlimited ? 'Unlimited' : '$' + metrics.maxProfit.toFixed(0)}
- Max Loss: ${metrics.isMaxLossUnlimited ? 'Unlimited' : '$' + Math.abs(metrics.maxLoss).toFixed(0)}
- Breakeven(s): ${metrics.breakevens.map(b => '$' + b.toFixed(2)).join(', ')}
- Net Premium: $${metrics.netPremium.toFixed(2)} (${metrics.netPremium >= 0 ? 'credit' : 'debit'})` : ''}
`;

      const systemPrompt = `You are an expert options trading analyst. Analyze the given options strategy and provide optimization suggestions. Be specific, actionable, and consider risk/reward tradeoffs.
...
Return a JSON object with this exact structure:
{
  ...
  "overallAssessment": "Brief overall strategy assessment",
  "riskScore": 1-10 (10 = highest risk),
  "potentialImprovement": "Estimated improvement if suggestions followed"
}

Consider:
1. Greeks balance (delta-neutral, gamma risk, theta decay)
2. Risk/reward ratio optimization
3. Strike and width adjustments
4. Rolling opportunities
5. Hedging suggestions
6. Position sizing
7. Volatility exposure

Provide 3-5 actionable suggestions. Be concise but specific.`;
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: strategyContext }
          ],
      ...
      if (!response.ok) {
        ...
        console.error("[OPTIMIZE] OpenAI API error:", response.status, errorData);
        return res.status(502).json({ 
          error: "Failed to analyze strategy with AI" 
        });
      }
      ...
      console.log(`[OPTIMIZE] Analysis complete for ${symbol}: ${analysis.suggestions?.length || 0} suggestions`);
