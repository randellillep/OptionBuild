  ...
  useEffect(() => {
    ...
    trades.forEach(trade => {
      ...
      const updatedLegs = rawLegs.map(leg => {
        ...
        if (isLegFullyClosed(leg)) {
          // Retroactively correct auto-generated close prices that used the stale saved market
          // price instead of intrinsic value. We identify auto-generated closes by the `exp-`
          // id prefix set when auto-closing. User-manually-set entries won't have this prefix.
          if (!isExpired) return leg;
          const entries = leg.closingTransaction?.entries || [];
          const isAutoGenerated = entries.length > 0 && entries.every((e: any) => e.id?.startsWith('exp-'));
          if (!isAutoGenerated) return leg;
          const correctPrice = leg.type === 'call'
            ? Math.max(0, price - leg.strike)
            : Math.max(0, leg.strike - price);
          const existingPrice = leg.closingTransaction?.closingPrice ?? 0;
          if (Math.abs(existingPrice - correctPrice) < 0.005) return leg; // Already correct
          // Price was wrong — recalculate with intrinsic value
          hasChanges = true;
          return {
            ...leg,
            marketBid: correctPrice,
            marketAsk: correctPrice,
            marketMark: correctPrice,
            marketLast: correctPrice,
            closingTransaction: {
              ...leg.closingTransaction,
              closingPrice: correctPrice,
              entries: entries.map((e: any) => ({
                ...e,
                closingPrice: correctPrice,
                underlyingPriceAtClose: price,
              })),
            },
          };
        }
        ...
        if (isExpired) {
          hasChanges = true;
          // At expiration an option settles at its intrinsic value (ITM) or $0 (OTM).
          // Never use savedMarket here — it reflects the price when the trade was saved,
          // not the settlement price, which leads to incorrect closing prices (e.g. showing
          // the original entry price instead of the actual expiration value).
          const closingPrice = leg.type === 'call'
            ? Math.max(0, price - leg.strike)
            : Math.max(0, leg.strike - price);
          return {
            ...leg,
  ...
  const calculateTotalReturn = (trade: SavedTrade): { value: number; percent: number } => {
    ...
    const legs = rawLegs.map(leg => {
      ...
      if (isExpired && !matchingQuote && !leg.closingTransaction?.isEnabled) {
        // At expiration options settle at intrinsic value (ITM) or $0 (OTM).
        const closingPrice = leg.type === 'call'
