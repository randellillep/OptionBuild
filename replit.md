...
- **Multi-day intraday heatmap** (1-7 DTE): Shows only trading session hours (9:30 AM–4:00 PM ET) across multiple days, matching OptionStrat's behavior. When market is closed, starts from next trading day's open. Each day's session is a separate block with day label headers ("10 M", "11 T"). All ET timezone conversions use `Intl.DateTimeFormat` with `timeZone: 'America/New_York'` for timezone-safe computation regardless of user's local timezone. `etTimeToUtcMs()` helper converts ET date/time to UTC ms via offset correction.
