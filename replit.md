...
- Expired legs are auto-closed using intrinsic value at expiration: call: max(0, underlying-strike), put: max(0, strike-underlying). OTM options close at $0. `savedMarket` is intentionally NOT used — it reflects the price when the trade was saved (often the entry price), which is wrong for the settlement price. Auto-generated closes carry `id: "exp-{legId}"` so they can be retroactively corrected if needed. `expirationDate` and `type` stored in closing entries. User can edit closing price if needed.
